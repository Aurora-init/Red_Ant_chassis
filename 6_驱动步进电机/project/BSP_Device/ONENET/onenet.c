...
_Bool OneNet_DevLink(void)
{
	...
	if(MQTT_PacketConnect(PROID, AUTH_INFO, DEVID, 256, 0, MQTT_QOS_LEVEL0, NULL, NULL, 0, &mqttPacket) == 0)
	{
		...
		if(dataPtr != NULL)
		{
			...
			if(MQTT_UnPacketRecv(dataPtr) == MQTT_PKT_CONNACK)
			{
				...
				{
					case 0:	usb_printf("Tips:	连接成功\r\n");status = 0;								break;
					case 1:	usb_printf("WARN:	连接失败：协议错误\r\n");									break;
					case 2:	usb_printf("WARN:	连接失败：非法的clientid\r\n");						break;
					case 3:	usb_printf("WARN:	连接失败：服务器失败\r\n");								break;
					case 4:	usb_printf("WARN:	连接失败：用户名或密码错误\r\n");					break;
					case 5:	usb_printf("WARN:	连接失败：非法链接(比如token非法)\r\n");	break;
					default:usb_printf("ERR:	连接失败：未知错误\r\n");									break;
				}
			}
		}
	...
	{	
		usb_printf("平台无响应\r\n");
		usb_printf("WARN:	MQTT_PacketConnect Failed\r\n");
	}
	return status;
}
...
{
	MQTT_PACKET_STRUCTURE mqttPacket = {NULL, 0, 0, 0};			//协议包				
	usb_printf("Publish Topic: %s, Msg: %s\r\n", topic, msg);
...
void OneNet_SendData(void)
{
	...
	if(body_len)
	{
		if(MQTT_PacketSaveData(DEVID, body_len, NULL, 3, &mqttPacket) == 0)					//封包				
		{
			...
			usb_printf("Send %d Bytes\r\n", mqttPacket._len);
			MQTT_DeleteBuffer(&mqttPacket);																						//删包
		}
		else
			usb_printf("WARN:	MQTT_NewBuffer Failed\r\n");
	}
}
...
void OneNet_RevPro(unsigned char *cmd)
{
	...
	switch(type)
	{
		...
		case MQTT_PKT_CMD:		
			...
			if(result == 0)
			{
				usb_printf("cmdid: %s, req: %s, req_len: %d\r\n", cmdid_topic, req_payload, req_len);
				if(MQTT_PacketCmdResp(cmdid_topic, req_payload, &mqttPacket) == 0)		//命令回复组包
				{
					usb_printf("Tips:	Send CmdResp\r\n");
